...
&& isset($_GET["aplicacion"]) && isset($_GET["fecha_aplicacion"])) {
  ...
  $insert="INSERT INTO primera_aplicacion(id_vacuna, id_cartilla, aplicacion, fecha_aplicacion) VALUES ('{$id_vacuna}','{$id_cartilla}','{$aplicacion}','{$fecha_aplicacion}')";
  $resultado_insert=mysqli_query($conexion,$insert);

  if($resultado_insert){
    $consulta="SELECT LAST_INSERT_ID() FROM primera_aplicacion";
    $resultado=mysqli_query($conexion,$consulta);

    if($registro=mysqli_fetch_assoc($resultado)){
      $json['primera_aplicacion'][]=$registro;
    }
